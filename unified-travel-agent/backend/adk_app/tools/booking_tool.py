...
def confirm_booking_adk(
    city: str, flight_no: str, hotel_name: str, contact_info: str, tool_context: ToolContext
) -> dict:
    """
            ...
            if flight_no:
                ...
                if row:
                    if _already_booked(cur, session_id, "flight", row["id"]):
                        ...
                        cur.execute(
                            "INSERT INTO bookings (session_id, user_id, booking_type, reference_id, contact_info) "
                            "VALUES (%s, %s, 'flight', %s, %s)",
                            (session_id, user_id, row["id"], contact_info or None),
                        )
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT b.booking_type, b.created_at,
